...
namespace cfgo
{
    namespace gst
    {
        ...
        : m_owner(owner), m_client(get_client(client_handle)), m_sub_timeout(sub_timeout), m_read_timeout(m_read_timeout), m_ready(false)
        {
            cfgo_pattern_parse(pattern_json, m_pattern);
            cfgo_req_types_parse(req_types_str, m_req_types);
            asio::co_spawn(asio::get_associated_executor(m_client->execution_context()), _loop(), asio::detached);
        }
        ...
        {
            return std::make_unique<CfgoSrc>(owner, client_handle, pattern_json, req_types_str, sub_timeout, read_timeout);
        }
