...
func getPictureOrderCount(buf []byte, sps *h264.SPS) (uint32, error) {
	buf = buf[1:]
	lb := len(buf)
...
func getPictureOrderCountDiff(a uint32, b uint32, sps *h264.SPS) int32 {
...
// NewH264DTSExtractor allocates a DTSExtractor.
func NewH264DTSExtractor() *DTSExtractor {
	return &DTSExtractor{
		pocIncrement: 2,
	}
}
