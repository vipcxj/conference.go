...
func generateMediaPlaylistFMP4(
	...
	for i, sog := range segments {
		...
		case *muxerSegmentFMP4:
			...
			if variant == gohlslib.MuxerVariantLowLatency && (len(segments)-i) <= 2 {
				for _, part := range seg.parts {
...
func (s *muxerServer) hasContent() bool {
	if s.variant == gohlslib.MuxerVariantFMP4 {
...
func (s *muxerServer) handle(w http.ResponseWriter, r *http.Request) {
	...
	case s.variant != gohlslib.MuxerVariantMPEGTS && name == s.prefix+"_init.mp4":
		s.handleInitFile(w)

	case (s.variant != gohlslib.MuxerVariantMPEGTS && strings.HasSuffix(name, ".mp4")) ||
		(s.variant == gohlslib.MuxerVariantMPEGTS && strings.HasSuffix(name, ".ts")):
...
func (s *muxerServer) handleMediaPlaylist(msn string, part string, skip string, w http.ResponseWriter) {
	...
	if s.variant == gohlslib.MuxerVariantLowLatency {
...
func (s *muxerServer) handleSegmentOrPart(fname string, w http.ResponseWriter) {
	...
	case strings.HasPrefix(fname, s.prefix+"_"+"seg"):
		...
		w.Header().Set(
			"Content-Type",
			func() string {
				if s.variant == gohlslib.MuxerVariantMPEGTS {
					return "video/MP2T"
				}
				return "video/mp4"
			}(),
	...
	case s.variant == gohlslib.MuxerVariantLowLatency && strings.HasPrefix(fname, s.prefix+"_"+"part"):
		s.mutex.Lock()
...
func (s *muxerServer) publishSegment(segment muxerSegment) error {
	err := func() error {
		...
		if s.variant == gohlslib.MuxerVariantLowLatency && len(s.segments) == 0 {
			for i := 0; i < 7; i++ {
		...
		if s.variant != gohlslib.MuxerVariantMPEGTS && (s.init == nil || segment.isForceSwitched()) {
			if s.init != nil {
				s.init.Remove()
				s.init = nil
			}
