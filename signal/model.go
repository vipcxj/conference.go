...
import (
	...
	_ "github.com/pion/interceptor/pkg/jitterbuffer"
...
func (ctx *SignalContext) OnPingMessage(message *model.PingMessage) {
	ctx.Sugar().Debugf("receive kafka ping msg %v from socket %v in node %v", message.GetMsgId(), message.GetRouter().GetSocketFrom(), message.GetRouter().GetNodeFrom())
	if !ctx.matchRoom(message) || !ctx.matchSocketTo(message, true) {
		return
	}
	err := ctx.emit("ping", proto.ToClientMessage(message))
	if err == nil {
		ctx.Sugar().Debugf("success to send ping msg %v to client %v", message.GetMsgId(), ctx.Id)
	} else {
		ctx.Sugar().Debugf("failed to send ping msg %v to client %v, %v", message.GetMsgId(), ctx.Id, err)
	}
}

func (ctx *SignalContext) OnPongMessage(message *model.PongMessage) {
	ctx.Sugar().Debugf("receive kafka pong msg %v from socket %v in node %v", message.GetMsgId(), message.GetRouter().GetSocketFrom(), message.GetRouter().GetNodeFrom())
	if !ctx.matchRoom(message) || !ctx.matchSocketTo(message, true) {
		return
	}
	err := ctx.emit("pong", proto.ToClientMessage(message))
	if err == nil {
		ctx.Sugar().Debugf("success to send pong msg %v to client %v", message.GetMsgId(), ctx.Id)
	} else {
		ctx.Sugar().Debugf("failed to send pong msg %v to client %v, %v", message.GetMsgId(), ctx.Id, err)
	}
}
...
func createPeer(conf *config.ConferenceConfigure, logger logging.LeveledLogger) (*webrtc.PeerConnection, error) {
	...
	// if conf.WebRTC.JitterBuffer {
	// 	var jitterbufferFactory *jitterbuffer.InterceptorFactory
	// 	if logger != nil {
	// 		jitterbufferFactory, err = jitterbuffer.NewInterceptor(jitterbuffer.Log(logger))
	// 	} else {
	// 		jitterbufferFactory, err = jitterbuffer.NewInterceptor()
	// 	}
	// 	if err != nil {
	// 		return nil, fmt.Errorf("unable to create jitterbuffer interceptor factory, %w", err)
	// 	}
	// 	i.Add(jitterbufferFactory)
	// }
	...
	if err != nil {
		return nil, fmt.Errorf("unable to create interval pli interceptor factory, %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("unable to create peer, %w", err)
	}
	return peer, err
}
...
func (ctx *SignalContext) makeSurePeer() (peer *webrtc.PeerConnection, err error) {
	...
	if ctx.Peer != nil {
		...
		peer, err = createPeer(ctx.Global.Conf(), log.NewPionSugar(ctx.Sugar()))
		if err != nil {
			return
		}
